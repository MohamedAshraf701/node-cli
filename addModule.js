...
program
  ...
  .action((name, options) => {
    ...
    if (!options.express && !options.fastify) {
      console.error(
        "Please choose one of the following options: --express or --fastify"
      );
      process.exit(1);
    }
    if (options.express && options.fastify) {
      console.error(
        "Please choose only one option: either --express or --fastify, not both."
      );
      process.exit(1);
    }
    if (!options.javascript && !options.typescript) {
      console.error(
        "Please choose one of the following options: --javascript or --typescript"
      );
      process.exit(1);
    }
    if (options.javascript && options.typescript) {
    ...
    if (!name) {
      ...
      console.log(
        "Please choose one of the following options: --mongo or --seque"
      );
